...
export default function LandingPage({ setActiveView, onOpenCompanyPage, onSelectJob, onOpenAuth, user }: LandingPageProps) {
  ...
  const handleHeroSearch = (event: FormEvent) => {
    ...
    window.sessionStorage.setItem("aijobs_search_query", query);
    ...
    try {
      ...
      try {
        const parseRes = await fetch("/api/resume/parse", {
          ...
          body: JSON.stringify({ userId: tempId, resumeUrl: "https://storage.googleapis.com/temp/resume.pdf", fileName: file.name, fileBase64, fileType: file.type })
        });
        if (parseRes.ok) parseJson = await parseJsonResponse(parseRes).catch(() => null);
      } catch (parseError) {
        console.warn("Resume parsing notice:", parseError);
      }
    ...
    <div className="relative min-h-screen w-full overflow-x-hidden bg-white text-slate-900 selection:bg-blue-600 selection:text-white">
      ...
      {isSmartOnboarding && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#07152F]/90 p-4 backdrop-blur-xl">
          <div className="w-full max-w-md rounded-[28px] border border-white/10 bg-white p-7 text-center shadow-2xl">
        ...
        {!reduceMotion && (
          <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
            {Array.from({ length: 18 }).map((_, index) => (
              <motion.span key={index} className="absolute h-1 w-1 rounded-full bg-cyan-300/50 shadow-[0_0_16px_rgba(34,211,238,.8)]" style={{ left: `${(index * 17) % 96}%`, top: `${12 + ((index * 29) % 76)}%` }} animate={{ y: [0, -18, 0], opacity: [0.18, 0.7, 0.18] }} transition={{ duration: 4 + (index % 5), repeat: Infinity, delay: index * 0.16, ease: "easeInOut" }} />
            ))}
          </div>
        )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
            <div className="max-w-2xl">
              <span className="text-xs font-extrabold uppercase tracking-[0.18em] text-blue-600">Find your direction</span>
              <h2 className="mt-3 text-3xl sm:text-4xl font-black tracking-tight text-slate-950">Explore job categories</h2>
              <p className="mt-3 text-sm sm:text-base text-slate-500">Browse live opportunities by the work you want to do—without invented job counts.</p>
            </div>
            <button onClick={() => setActiveView("public-jobs")} className="inline-flex items-center gap-2 text-sm font-extrabold text-blue-600 hover:text-blue-700">View all live jobs <ArrowRight className="w-4 h-4" /></button>
          </div>
          <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {POPULAR_CATEGORIES.map((category) => (
              <button key={category.id} onClick={() => handleCategoryClick(category.query)} className="group rounded-[22px] border border-slate-200 bg-slate-50 p-4 text-left transition hover:-translate-y-0.5 hover:border-blue-200 hover:bg-blue-50/60 hover:shadow-lg">
                <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white text-blue-600 shadow-sm"><Briefcase className="w-4 h-4" /></div>
                <div className="mt-4 text-sm font-extrabold text-slate-900 group-hover:text-blue-700">{category.title}</div>
                <div className="mt-1 text-[11px] font-semibold text-slate-400">Explore openings</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 bg-slate-50 border-y border-slate-200/70">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
            <div>
              <span className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.18em] text-blue-600"><Clock className="w-3.5 h-3.5" />Real data only</span>
              <h2 className="mt-3 text-3xl sm:text-4xl font-black tracking-tight text-slate-950">Latest job openings</h2>
              <p className="mt-3 text-sm sm:text-base text-slate-500">These cards come from the current live jobs source used by AIJOBS.</p>
            </div>
            <button onClick={() => setActiveView("public-jobs")} className="min-h-11 rounded-2xl bg-[#2563EB] px-5 text-sm font-extrabold text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-700">View All Jobs</button>
          </div>
            ...
            {loadingJobs ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                {liveJobs.slice(0, 6).map((job) => (
                  <article key={job.id} className="group flex min-h-[270px] flex-col justify-between rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl">
                    <div>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0"><h3 className="text-base font-black text-slate-950 line-clamp-2">{job.title}</h3><div className="mt-2 flex items-center gap-2 text-xs font-semibold text-slate-500"><Building2 className="w-3.5 h-3.5 shrink-0" /><span className="truncate">{job.companyName || job.company || "Verified Employer"}</span></div></div>
                        {job.jobType && <span className="shrink-0 rounded-xl bg-blue-50 px-2.5 py-1 text-[10px] font-extrabold text-blue-700">{job.jobType}</span>}
                      </div>
                      <div className="mt-5 grid grid-cols-1 gap-2 text-xs text-slate-600">
                        {job.location && <div className="flex items-center gap-2"><MapPin className="w-3.5 h-3.5 text-slate-400" /><span>{job.location}</span></div>}
                        {job.experience && <div className="flex items-center gap-2"><Briefcase className="w-3.5 h-3.5 text-slate-400" /><span>{job.experience}</span></div>}
                      </div>
                      {(job.salary || job.postedDate) && <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-3 py-2.5 text-xs"><span className="font-extrabold text-slate-800">{job.salary || ""}</span><span className="text-slate-400">{job.postedDate || ""}</span></div>}
                    </div>
                    <div className="mt-5 grid grid-cols-2 gap-2">
                      <button onClick={() => openJob(job)} className="min-h-10 rounded-xl bg-slate-100 px-3 text-xs font-extrabold text-slate-700 transition hover:bg-slate-200">View Details</button>
                      <button onClick={() => openJob(job)} className="min-h-10 rounded-xl bg-[#2563EB] px-3 text-xs font-extrabold text-white shadow-md shadow-blue-500/20 transition hover:bg-blue-700">Quick Apply</button>
                    </div>
                  </article>
                ))}
              </div>
      ...
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div className="rounded-[30px] bg-[#07152F] p-7 sm:p-9 text-white shadow-2xl">
              <div className="inline-flex items-center gap-2 rounded-full bg-cyan-300/10 px-3 py-1.5 text-xs font-extrabold text-cyan-300"><Zap className="w-4 h-4" />Quick Apply</div>
              <h2 className="mt-4 text-3xl sm:text-4xl font-black tracking-tight">Less repetition. Faster applications.</h2>
              <p className="mt-3 text-sm sm:text-base leading-7 text-slate-400">Browse live jobs, open the role, review details and continue through the existing application flow. Creating a profile makes repeat applications easier while quick apply remains available from live jobs.</p>
              <button onClick={() => setActiveView("public-jobs")} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-2xl bg-white px-5 text-sm font-extrabold text-[#07152F]">Browse Jobs <ArrowRight className="w-4 h-4" /></button>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              {[{title:"Save profile",text:"Keep your candidate details ready for future applications.",icon:FileText},{title:"Save applications",text:"Stay organized as you apply to more roles.",icon:Briefcase},{title:"Resume visibility",text:"A complete resume helps recruiters understand your experience.",icon:Upload},{title:"Better matching",text:"Use profile context to discover more relevant opportunities.",icon:Sparkles}].map(({title,text,icon:Icon}) => <div key={title} className="rounded-[24px] border border-slate-200 bg-slate-50 p-5"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-white text-blue-600 shadow-sm"><Icon className="w-4 h-4" /></div><h3 className="mt-4 text-sm font-black text-slate-950">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{text}</p></div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-[1fr_auto] gap-8 items-center rounded-[30px] border border-slate-200 bg-white p-7 sm:p-9 shadow-sm">
            <div><span className="text-xs font-extrabold uppercase tracking-[0.18em] text-blue-600">Resume & profile</span><h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950">Make your profile ready before the right job appears.</h2><p className="mt-3 max-w-2xl text-sm sm:text-base text-slate-500">Upload your resume after signing in, keep your candidate profile current and make repeat applications easier.</p></div>
            <div className="flex flex-col sm:flex-row lg:flex-col gap-3"><button onClick={openCandidateSignup} className="min-h-12 rounded-2xl bg-blue-600 px-6 text-sm font-extrabold text-white shadow-lg shadow-blue-500/20">Create Free Profile</button><button onClick={handleResumeButtonClick} className="min-h-12 rounded-2xl border border-slate-200 bg-slate-50 px-6 text-sm font-extrabold text-slate-800">Upload Resume</button></div>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-slate-950 text-slate-300 border-t border-white/10 pt-14 pb-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-9">
            <div className="lg:col-span-2"><AIJobsLogo className="h-9 w-auto text-white" /><p className="mt-4 max-w-md text-sm leading-6 text-slate-400">AIJOBS connects candidates with live recruitment opportunities through a modern job discovery and application experience.</p><div className="mt-4 inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.05] px-3 py-2 text-xs font-bold text-cyan-300"><ShieldCheck className="w-4 h-4" />No candidate placement charges</div></div>
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between border-t border-white/10 pt-7 text-xs text-slate-500"><span>© {new Date().getFullYear()} AIJOBS. All rights reserved.</span><span>Find Smarter. Hire Faster.</span></div>
        </div>
      </footer>
      ...
      {activeLegalDoc && <LegalModal docType={activeLegalDoc} onClose={() => setActiveLegalDoc(null)} />}
    </div>
  );
}
