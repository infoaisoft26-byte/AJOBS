...
export const AIJobs3DIntro: React.FC<AIJobs3DIntroProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const updateLoop = () => {
      ...
      // Smooth fade out into homepage at 14.5s
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 110) {
            ...
            ctx.strokeStyle = `rgba(14, 165, 233, ${(1 - dist / 110) * 0.2})`;
            ctx.lineWidth = 0.75;
            ctx.stroke();
          }
        }
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-[99999] bg-[#030712] text-white flex flex-col justify-between overflow-hidden select-none transition-opacity duration-700 ${
        isFadingOut ? "opacity-0 pointer-events-none" : "opacity-100"
      }`}
    >
      {/* Dynamic Background Atmosphere */}
      <div className="absolute inset-0 pointer-events-none">
        {/* Deep luxury navy & royal blue gradient */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_40%,rgba(15,32,67,0.75)_0%,rgba(6,13,31,0.92)_55%,rgba(3,7,18,1)_100%)]" />

        {/* Dawn Horizon Glow in Scene 1 */}
        {sceneNum === 1 && (
          <div className="absolute bottom-0 left-0 right-0 h-80 bg-gradient-to-t from-amber-500/15 via-blue-500/10 to-transparent blur-3xl transition-opacity duration-1000" />
        )}

        {/* AI Blue Ambient Bloom in Scenes 3, 4, 5 */}
        {sceneNum >= 3 && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] bg-blue-600/15 rounded-full blur-3xl pointer-events-none transition-all duration-1000" />
        )}
      </div>
      ...
      {/* Audio Unlock Prompt if blocked by browser policy */}
      {audioPromptNeeded && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-40">
          <button
            onClick={handleAudioUnlock}
            className="flex items-center space-x-3 px-5 py-2.5 rounded-full bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-600 text-white font-medium text-xs shadow-2xl shadow-blue-500/30 border border-blue-400/40 hover:scale-105 transition active:scale-95 animate-bounce cursor-pointer"
          >
      ...
      <div className="relative z-20 flex-1 flex items-center justify-center px-6 sm:px-12 max-w-6xl mx-auto w-full">
        ...
        {sceneNum === 1 && (
          <div className="flex flex-col items-center justify-center text-center space-y-6 w-full max-w-4xl animate-fadeIn">
            <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-slate-900/90 border border-amber-500/30 text-xs text-amber-200 font-mono shadow-xl backdrop-blur-md">
              <Building2 className="w-3.5 h-3.5 text-amber-400" />
              <span>Modern Indian Business District • Dawn Sunrise</span>
            </div>

            {/* Realistic Professional Profiles entering the workplace */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 w-full">
              <div className="p-4 rounded-2xl bg-gradient-to-b from-slate-900/95 to-slate-950/95 border border-slate-700/80 flex flex-col items-center text-center space-y-2.5 shadow-xl backdrop-blur-md">
                <div className="w-12 h-12 rounded-full bg-blue-600/20 border border-blue-500/40 flex items-center justify-center text-blue-400 font-semibold text-sm">
                  YP
                </div>
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-b from-slate-900/95 to-slate-950/95 border border-slate-700/80 flex flex-col items-center text-center space-y-2.5 shadow-xl backdrop-blur-md">
                <div className="w-12 h-12 rounded-full bg-purple-600/20 border border-purple-500/40 flex items-center justify-center text-purple-400 font-semibold text-sm">
                  PL
                </div>
                <div className="space-y-0.5">
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-b from-slate-900/95 to-slate-950/95 border border-slate-700/80 flex flex-col items-center text-center space-y-2.5 shadow-xl backdrop-blur-md">
                <div className="w-12 h-12 rounded-full bg-emerald-600/20 border border-emerald-500/40 flex items-center justify-center text-emerald-400 font-semibold text-sm">
                  EP
                </div>
                <div className="space-y-0.5">
                  <div className="text-xs font-semibold text-white">Senior Architect</div>
                  <div className="text-[11px] text-slate-400">12+ Years Experience</div>
                </div>
              </div>

              <div className="p-4 rounded-2xl bg-gradient-to-b from-slate-900/95 to-slate-950/95 border border-slate-700/80 flex flex-col items-center text-center space-y-2.5 shadow-xl backdrop-blur-md">
                <div className="w-12 h-12 rounded-full bg-amber-600/20 border border-amber-500/40 flex items-center justify-center text-amber-400 font-semibold text-sm">
                  FG
                </div>
                <div className="space-y-0.5">
            ...
            <div className="space-y-1 pt-2">
              ...
              <p className="text-xs font-mono tracking-[0.25em] text-blue-400 uppercase">
                India's Next Generation of Leaders
              </p>
            </div>
          </div>
        )}
        ...
        {sceneNum === 2 && (
          <div className="flex flex-col items-center justify-center text-center space-y-6 w-full max-w-4xl animate-fadeIn">
            <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-slate-900/90 border border-blue-500/30 text-xs text-cyan-300 font-mono shadow-xl backdrop-blur-md">
              <Briefcase className="w-3.5 h-3.5 text-cyan-400" />
              <span>Real Job Applications • Verified Career Profiles</span>
            </div>

            {/* Candidate Workspace & Interview Preparation Cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
              <div className="p-5 rounded-2xl bg-slate-900/90 border border-blue-500/30 flex flex-col text-left space-y-3 shadow-xl backdrop-blur-md">
              ...
              <div className="p-5 rounded-2xl bg-slate-900/90 border border-purple-500/30 flex flex-col text-left space-y-3 shadow-xl backdrop-blur-md">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono text-purple-400 uppercase tracking-wider">Interview Room</span>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 font-medium">Ready</span>
                </div>
                <div className="space-y-1">
                  <div className="text-sm font-semibold text-white">Direct Enterprise Interaction</div>
                  <div className="text-xs text-slate-300 font-light">
                    Candidate entering formal interview with confidence and portfolio clarity.
                  </div>
                </div>
              </div>

              <div className="p-5 rounded-2xl bg-slate-900/90 border border-emerald-500/30 flex flex-col text-left space-y-3 shadow-xl backdrop-blur-md">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono text-emerald-400 uppercase tracking-wider">Job Matching</span>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 font-medium">98% Match</span>
                </div>
                <div className="space-y-1">
                  <div className="text-sm font-semibold text-white">Targeted Opportunities</div>
                  <div className="text-xs text-slate-300 font-light">
                    Matching verified skills directly to top enterprise and high-growth hiring needs.
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <h2 className="text-xl sm:text-3xl font-serif text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-300 to-sky-400 tracking-wider">
                STARTS HERE
              </h2>
            </div>
          </div>
        ...
        {sceneNum === 3 && (
          <div className="flex flex-col items-center justify-center text-center space-y-6 w-full max-w-5xl animate-fadeIn">
            <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-slate-900/90 border border-blue-500/40 text-xs text-blue-300 font-mono shadow-xl backdrop-blur-md">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 w-full">
              {/* Node 1: Candidate */}
              <div className="px-4 py-3 rounded-xl bg-slate-900/90 border border-blue-500/40 flex flex-col items-center space-y-1 shadow-lg backdrop-blur-md">
              ...
              {/* Node 2: Skills */}
              <div className="px-4 py-3 rounded-xl bg-slate-900/90 border border-indigo-500/40 flex flex-col items-center space-y-1 shadow-lg backdrop-blur-md">
              ...
              {/* Node 3: Job */}
              <div className="px-4 py-3 rounded-xl bg-slate-900/90 border border-cyan-500/40 flex flex-col items-center space-y-1 shadow-lg backdrop-blur-md">
              ...
              {/* Node 4: Recruiter */}
              <div className="px-4 py-3 rounded-xl bg-slate-900/90 border border-emerald-500/40 flex flex-col items-center space-y-1 shadow-lg backdrop-blur-md">
              ...
              {/* Node 5: Company */}
              <div className="px-4 py-3 rounded-xl bg-slate-900/90 border border-purple-500/40 flex flex-col items-center space-y-1 shadow-lg backdrop-blur-md">
            ...
            <div className="space-y-1 pt-1">
              <h1 className="text-2xl sm:text-4xl font-serif font-bold text-white tracking-widest">
                SMARTER MATCHING
              </h1>
              <h2 className="text-xl sm:text-3xl font-serif text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-sky-300 to-emerald-400 tracking-wider">
                FASTER HIRING
              </h2>
            </div>
          </div>
        ...
        {sceneNum === 4 && (
          <div className="flex flex-col items-center justify-center text-center space-y-6 w-full max-w-4xl animate-fadeIn">
            <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-slate-900/90 border border-emerald-500/40 text-xs text-emerald-300 font-mono shadow-xl backdrop-blur-md">
              ...
              <div className="p-6 rounded-2xl bg-slate-900/95 border border-slate-700/80 flex flex-col text-left space-y-4 shadow-2xl backdrop-blur-md">
                ...
                <div className="pt-1">
                  {recruiterAction === "initial" && (
                    <div className="w-full py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-medium text-xs text-center transition shadow-lg flex items-center justify-center space-x-2">
