...
app.post("/api/admin/create-admin", async (req, res) => {
  try {
    ...
    if (!authHeader.startsWith("Bearer ")) {
      return res.status(401).json({ success: false, error: "Super Admin authentication is required." });
    }
    ...
    if (!requesterUserDoc.exists && !requesterAdminDoc.exists) {
      return res.status(403).json({ success: false, error: "Super Admin profile not found." });
    }
    if (!["superadmin", "super_admin"].includes(requesterRole) || ["disabled", "suspended", "inactive"].includes(String(requester.status || "active").toLowerCase())) {
      return res.status(403).json({ success: false, error: "Only an active Super Admin can provision Admin accounts." });
    }
    ...
    try {
      ...
      userRecord = await adminAuth.updateUser(userRecord.uid, {
        displayName: name || userRecord.displayName || "Admin Desk",
        disabled: false
      });
    ...
    if (!existingAccount) profile.createdAt = now;
    ...
    return res.json({ success: true, message: existingAccount ? "Existing Admin account synchronized successfully." : "Admin account created successfully.", uid: userRecord.uid });
  } catch (err: any) {
